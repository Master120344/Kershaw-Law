...
function initPageLoad() {
    const splashLoader = 
document.getElementById('splash-loader');
...
(parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--loader-display-duration')) 
|| 0.1) * 1000;
...
document.getElementById('page-transition-loader');
    const mainContent = 
document.getElementById('main-content');
    if (!transitionLoader || !mainContent) 
return;
...
window.location.pathname.replace(/\/$/, "");
            const destPathObj = new URL(dest, 
window.location.href);
            const destPath = 
destPathObj.pathname.replace(/\/$/, "");

            // Prevents re-animation for same 
page links
...
observerInstance.unobserve(entry.target);
    ...
    }, { root: null, rootMargin: '0px 0px -15% 
0px', threshold: 0.1 });
...
document.getElementById('site-header');
    ...
    const handleScroll = debounce(() => {
        const nowST = window.pageYOffset || 
document.documentElement.scrollTop;
        if (Math.abs(lastScrollTop - nowST) <= 
delta) return;

        if (nowST > lastScrollTop && nowST > 
headerHeight) {
            
header.classList.add('scrolled-down');
        } else if (nowST + window.innerHeight 
< document.documentElement.scrollHeight) {
            
header.classList.remove('scrolled-down');
        }
        lastScrollTop = nowST <= 0 ? 0 : 
nowST;
    }, 50);
...
document.getElementById('page-transition-loader');
    if (transitionLoader) 
transitionLoader.classList.add('hidden');
